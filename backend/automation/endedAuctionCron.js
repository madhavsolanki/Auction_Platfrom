...
import { calculateCommision } from "../controllers/commision.controller.js";
...
export const endedAuctionCron = () => {
  cron.schedule("*/1 * * * *", async() => {
    const now = new Date();
    console.log("Cron for Ended Auction is Running....");
    ...
    for(const auction of endedAuctions){
      try {
        const commisionAmount = await calculateCommision(auction._id);
        ...
        auctioneer.unpaidCommission = commisionAmount;

        if(highestBidder){
          auction.highestBidder = highestBidder.bidder.id;
          await auction.save();
          const bidder = User.findById(highestBidder.bidder.id);
          ...
          await User.findByIdAndUpdate(auctioneer._id, 
            {
              $inc: {
                unpaidCommission: commisionAmount
              }
            },
            {new : true}
          );
